...
/**
 * Deploys:
 *  1. HushBid — main auction contract (with World ID Router for sybil resistance)
...
async function main() {
  ...
  const WORLD_ID_ROUTER = getAddress(
    process.env.WORLD_ID_ROUTER || "0x469449f251692E0779667583026b5A1E99B72157"
  );
  ...
  // 1. Deploy HushBid (uses real World ID Router for Orb on-chain verification)
  ...
  try {
    execSync(
      `node ${updateScript} --hushBid ${hushBid.address} --priceNormalizer ${priceNormalizer.address}`,
      { cwd: ROOT, stdio: "inherit" }
    );
  } catch (err) {
    console.error("⚠️  Address update script failed — update manually:");
    console.error(`   node scripts/update-addresses.mjs --hushBid ${hushBid.address} --priceNormalizer ${priceNormalizer.address}`);
  }
}
