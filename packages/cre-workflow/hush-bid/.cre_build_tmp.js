...
var SIG_AUCTION_CREATED = keccak256(toBytes("AuctionCreated(uint256,address,address,uint256,uint8)"));
var SIG_BID_COMMITTED = keccak256(toBytes("BidCommitted(uint256,bytes32,uint64)"));
var SIG_AUCTION_SETTLED = keccak256(toBytes("AuctionSettled(uint256,address,uint256,bytes32)"));
var AUCTION_PHASE = {
  CREATED: 0,
  BIDDING: 1,
  REVEAL: 2,
...
var configSchema = exports_external.object({
  ...
  primaryChain: exports_external.object({
    chainSelectorName: exports_external.string(),
    auctionContract: exports_external.string(),
    priceNormalizer: exports_external.string(),
    creCoordinator: exports_external.string(),
    gasLimit: exports_external.string()
  }),
  convergence: exports_external.object({
  ...
  tokens: exports_external.object({
    ...
    weth: exports_external.string(),
    usdc: exports_external.string()
  }),
  secrets: exports_external.object({
    donEthPrivateKey: exports_external.string(),
    donWalletAddress: exports_external.string(),
    donEncryptionPrivateKey: exports_external.string(),
    pinataJwt: exports_external.string().optional()
  })
});
var readSecret = (runtime2, id) => {
  const map = {
    DON_ETH_PRIVATE_KEY: runtime2.config.secrets.donEthPrivateKey,
    DON_WALLET_ADDRESS: runtime2.config.secrets.donWalletAddress,
    DON_ENCRYPTION_PRIVATE_KEY: runtime2.config.secrets.donEncryptionPrivateKey,
    PINATA_JWT: runtime2.config.secrets.pinataJwt
  };
  const val = map[id];
  if (!val)
    throw new Error(`Secret '${id}' not found in config.secrets`);
  return val;
};
var getPrimaryEvm = (config) => {
  ...
  return {
    evmClient: new ClientCapability(network248.chainSelector.selector),
    chainSelector: network248.chainSelector.selector
  };
};
var getAuction = (runtime2, auctionId) => {
  const { evmClient } = getPrimaryEvm(runtime2.config);
  const { primaryChain } = runtime2.config;
  ...
  const result = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: primaryChain.auctionContract,
      data: callData
    }),
...
var findHighestBid = (runtime2, tokens, amounts) => {
  const { evmClient } = getPrimaryEvm(runtime2.config);
  const { primaryChain } = runtime2.config;
  const callData = encodeFunctionData({
  ...
  const result = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: primaryChain.priceNormalizer,
      data: callData
    }),
    blockNumber: LATEST_BLOCK_NUMBER
  }).result();
...
var getAuctionPhase = (runtime2, auctionId) => {
  const { evmClient } = getPrimaryEvm(runtime2.config);
  const { primaryChain } = runtime2.config;
  const callData = encodeFunctionData({
    abi: HushBidABI,
...
var getAuctionCounter = (runtime2) => {
  const { evmClient } = getPrimaryEvm(runtime2.config);
  const { primaryChain } = runtime2.config;
  const callData = encodeFunctionData({
    abi: HushBidABI,
...
var getBidCount = (runtime2, auctionId) => {
  const { evmClient } = getPrimaryEvm(runtime2.config);
  const { primaryChain } = runtime2.config;
  const callData = encodeFunctionData({
    abi: HushBidABI,
...
var getBidCommitment = (runtime2, auctionId, index) => {
  const { evmClient } = getPrimaryEvm(runtime2.config);
  const { primaryChain } = runtime2.config;
  const callData = encodeFunctionData({
    abi: HushBidABI,
  ...
  const result = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: primaryChain.creCoordinator,
      to: primaryChain.auctionContract,
      data: callData
    }),
    blockNumber: LATEST_BLOCK_NUMBER
...
var getSettlementBids = (runtime2, auctionId, privacyLevel) => {
  const privacyName = ["FULL_PRIVATE", "AUDITABLE"][privacyLevel] ?? `UNKNOWN(${privacyLevel})`;
  runtime2.log(`Auction #${auctionId}: ${privacyName} — collecting bids from IPFS via Confidential HTTP`);
  const bidCount = getBidCount(runtime2, auctionId);
  if (bidCount === 0n) {
    runtime2.log(`Auction #${auctionId}: no bids found`);
    return [];
  }
  runtime2.log(`Auction #${auctionId}: found ${bidCount} bid commitment(s) — reading from contract storage`);
  const bids = [];
  for (let idx = 0n;idx < bidCount; idx++) {
    try {
      const commitment = getBidCommitment(runtime2, auctionId, idx);
      if (!commitment.ipfsCid || commitment.ipfsCid === "") {
        runtime2.log(`Skipping bid #${idx} — no IPFS CID`);
        continue;
      }
      const cidStr = commitment.ipfsCid;
      runtime2.log(`Fetching encrypted envelope from IPFS: ${cidStr}`);
      const envelope = fetchBidEnvelopeFromIpfs(runtime2, cidStr);
      let metadata;
      if (envelope.encryptedPayload) {
        metadata = decryptMetadata(runtime2, envelope.encryptedPayload);
      } else if (envelope.encryptedData && envelope.bidder) {
        runtime2.log(`Bid #${idx}: v2 frontend envelope — DON cannot decrypt wallet-encrypted data, skipping`);
        continue;
      } else {
      ...
      bids.push({
        ...
        destinationAddress: metadata.destinationAddress ?? metadata.bidder,
        bidIndex: idx
      });
    } catch (err) {
      runtime2.log(`Failed to process bid #${idx}: ${err}`);
      continue;
    }
  }
  runtime2.log(`Auction #${auctionId}: collected ${bids.length} bids from IPFS`);
  return bids;
};
function base64Decode2(b64) {
  ...
  while (i2 < s.length) {
    const a = chars.indexOf(s[i2++]);
    const b = chars.indexOf(s[i2++]);
    const c = chars.indexOf(s[i2++]);
    const d = chars.indexOf(s[i2++]);
...
function decryptMetadata(runtime2, encryptedPayloadBase64) {
  const raw = base64Decode2(encryptedPayloadBase64);
  try {
    const maybeJson = new TextDecoder().decode(raw);
    const parsed2 = JSON.parse(maybeJson);
    if (parsed2 && typeof parsed2.bidder === "string" && typeof parsed2.amount === "string") {
      runtime2.log("Bid envelope is plaintext (test mode) — skipping decryption");
      return parsed2;
    }
  } catch {}
  const keyHex = runtime2.config.secrets.donEncryptionPrivateKey;
  ...
  for (let offset = 0;offset < ciphertext.length; offset += 32) {
    ...
    blockInput[64] = blockIdx >> 24 & 255;
    blockInput[65] = blockIdx >> 16 & 255;
    blockInput[66] = blockIdx >> 8 & 255;
    blockInput[67] = blockIdx & 255;
  ...
  runtime2.log(`Decrypted bid metadata (${json.length} chars)`);
...
function hexToUint8Array(hex) {
  ...
  for (let i2 = 0;i2 < bytes.length; i2++) {
    bytes[i2] = parseInt(clean2.slice(i2 * 2, i2 * 2 + 2), 16);
  }
  return bytes;
}
var fetchBidEnvelopeFromIpfs = (runtime2, cid) => {
...
var executeSettlement = (runtime2, auctionId, winnerBidIndex, winningBid, paymentToken, settlementHash, destinationAddress) => {
  const { evmClient } = getPrimaryEvm(runtime2.config);
  const { primaryChain } = runtime2.config;
  runtime2.log(`Settling auction #${auctionId} — bidIndex=${winnerBidIndex} bid=${winningBid} dest=${destinationAddress}`);
  const callData = encodeFunctionData({
    abi: HushBidABI,
  ...
  if (resp.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`Settlement tx failed: ${resp.errorMessage ?? TxStatus[resp.txStatus]}`);
  }
  const txHash = bytesToHex(resp.txHash ?? new Uint8Array(32));
  runtime2.log(`Settlement succeeded: ${txHash}`);
  return txHash;
};
var CONVERGENCE_EIP712_DOMAIN = {
  name: "CompliantPrivateTokenDemo",
  version: "0.0.1"
};
function signPrivateTransfer(runtime2, from2, to, token, amount) {
...
function signDigest(privKey, digest) {
  const { secp256k1 } = require_secp256k1();
...
var executePrivateTransfer = (runtime2, from2, to, token, amount) => {
  ...
  const auth = signPrivateTransfer(runtime2, from2, to, token, amountBigInt);
  runtime2.log(`Private transfer: ${amount} of ${token} from ${from2} → ${to}`);
  ...
  const txResult = JSON.parse(text);
  runtime2.log(`Private transfer complete: txId=${txResult.transaction_id}`);
  return { transactionId: txResult.transaction_id };
};
var settlePrivatePayments = (runtime2, auction, bids, winnerIndex) => {
  const donPrivKeyHex = readSecret(runtime2, "DON_ETH_PRIVATE_KEY");
  ...
  try {
    executePrivateTransfer(runtime2, donAddress, auction.sellerShieldedAddress, winnerBid.paymentToken, winnerBid.amount.toString());
    runtime2.log(`✅ Paid seller shielded address ${auction.sellerShieldedAddress}`);
  } catch (err) {
    runtime2.log(`❌ Failed to pay seller: ${err}`);
    throw err;
  }
  for (let i2 = 0;i2 < bids.length; i2++) {
    if (i2 === winnerIndex)
      continue;
    const loserBid = bids[i2];
    try {
      executePrivateTransfer(runtime2, donAddress, loserBid.destinationAddress, loserBid.paymentToken, loserBid.amount.toString());
      runtime2.log(`✅ Refunded loser bid #${i2}: ${loserBid.amount} of ${loserBid.paymentToken} → ${loserBid.destinationAddress}`);
    } catch (err) {
      runtime2.log(`❌ Failed to refund bid #${i2} to ${loserBid.destinationAddress}: ${err}`);
    }
  }
  try {
    executePrivateTransfer(runtime2, donAddress, winnerBid.destinationAddress, auction.assetContract, auction.tokenAmount.toString());
    runtime2.log(`✅ Delivered auctioned asset: ${auction.tokenAmount} of ${auction.assetContract} → ${winnerBid.destinationAddress}`);
  } catch (err) {
    runtime2.log(`❌ Failed to deliver auctioned asset to winner: ${err}`);
    throw err;
  }
  runtime2.log("Private payment settlement complete");
};
var onCronTrigger = (runtime2, _payload) => {
  runtime2.log("⏱  Cron: scanning for settleable auctions…");
  const { primaryChain } = runtime2.config;
  const now = BigInt(Math.floor(runtime2.now().getTime() / 1000));
  const auctionCount = getAuctionCounter(runtime2);
  if (auctionCount === 0n) {
    runtime2.log("No auctions found");
    return "no-auctions";
  }
  runtime2.log(`Found ${auctionCount} auction(s) — scanning for settleable ones`);
  const MAX_SETTLE_ATTEMPTS = 2;
  let settledCount = 0;
  let attemptCount = 0;
  const settlements = [];
  for (let i2 = auctionCount;i2 >= 1n; i2--) {
    if (attemptCount >= MAX_SETTLE_ATTEMPTS) {
      runtime2.log(`Reached max settlement attempts (${MAX_SETTLE_ATTEMPTS}) — stopping scan`);
      break;
    }
    const auctionId = i2;
    const auction = getAuction(runtime2, auctionId);
    if (now <= auction.revealEnd)
      continue;
    const phase = getAuctionPhase(runtime2, auctionId);
    if (phase === AUCTION_PHASE.SETTLED || phase === AUCTION_PHASE.COMPLETED || phase === AUCTION_PHASE.CANCELLED) {
      runtime2.log(`Auction #${auctionId}: phase=${phase} — already terminal, skipping`);
      continue;
    }
    runtime2.log(`Auction #${auctionId}: phase=${phase} — attempting settlement`);
    attemptCount++;
    const bids = getSettlementBids(runtime2, auctionId, auction.privacyLevel);
    if (bids.length === 0) {
      runtime2.log(`Auction #${auctionId}: no bids available for settlement — skipping`);
      continue;
    }
    runtime2.log(`Auction #${auctionId}: ${bids.length} decrypted bids`);
    ...
    let highestUsd;
    if (allSameToken) {
      runtime2.log(`All bids use same token ${tokens[0]} — comparing amounts directly`);
      let maxIdx = 0;
      ...
      highestUsd = amounts[maxIdx];
    } else {
      try {
        const result = findHighestBid(runtime2, tokens, amounts);
        winnerIndex = result.winnerIndex;
        highestUsd = result.highestUsd;
      } catch (err) {
        runtime2.log(`⚠️ PriceNormalizer reverted (stale feeds?): ${err} — using off-chain estimation`);
        const ETH_USD = 2500n * 10n ** 8n;
        const STABLE_USD = 1n * 10n ** 8n;
        const ethAddr = "0x0000000000000000000000000000000000000000";
        const { tokens: tokenCfg } = runtime2.config;
        ...
        for (let j = 1;j < usdValues.length; j++) {
          if (usdValues[j] > usdValues[maxIdx])
            maxIdx = j;
        }
        winnerIndex = BigInt(maxIdx);
    ...
    runtime2.log(`Winner: bidIndex=${winnerBid.bidIndex} — ${winnerBid.amount} of token ${winnerBid.paymentToken} (≈${allSameToken ? "raw" : "$"}${highestUsd}) dest=${winnerBid.destinationAddress}`);
    const settlementHash = keccak256(encodeAbiParameters([
      { type: "uint256" },
      { type: "uint256" },
      { type: "uint256" },
      { type: "address" },
    ...
    try {
      executeSettlement(runtime2, auctionId, winnerBid.bidIndex, winnerBid.amount, winnerBid.paymentToken, settlementHash, winnerBid.destinationAddress);
      txBroadcast = true;
    } catch (err) {
      runtime2.log(`⚠️ Settlement tx reverted for auction #${auctionId} (likely already settled): ${err}`);
      continue;
    }
    let paymentSuccess = false;
    try {
      settlePrivatePayments(runtime2, auction, bids, Number(winnerIndex));
      paymentSuccess = true;
    } catch (err) {
      runtime2.log(`⚠️ Private payment settlement failed: ${err}`);
      runtime2.log(`ℹ️ On-chain settlement succeeded — funds are safe. Private transfers can be retried manually.`);
    }
    settlements.push({
    ...
    if (paymentSuccess) {
      ...
      runtime2.log(`⚠️ Auction #${auctionId}: on-chain settled but private payments failed — not counting as fully settled`);
    }
  }
  runtime2.log(`Settlement sweep complete — settled ${settledCount} auction(s)`);
  return JSON.stringify({ settled: settledCount, settlements });
};
var onAuctionCreated = (runtime2, payload) => {
  const topics = payload.topics;
  if (topics.length < 2) {
    throw new Error("Invalid AuctionCreated event — expected ≥ 2 topics");
  }
  const auctionId = BigInt(bytesToHex(topics[1]));
  const seller = topics.length >= 3 ? "0x" + bytesToHex(topics[2]).slice(26) : zeroAddress;
  const decoded = decodeAbiParameters([
    { name: "assetContract", type: "address" },
    { name: "tokenAmount", type: "uint256" },
    { name: "privacyLevel", type: "uint8" }
  ], bytesToHex(payload.data));
  runtime2.log(`\uD83D\uDCE6 AuctionCreated #${auctionId} by ${seller} — ` + `asset=${decoded[0]} amount=${decoded[1]}, privacy=${decoded[2]}`);
  return auctionId.toString();
};
var onBidCommitted = (runtime2, payload) => {
  const topics = payload.topics;
  if (topics.length < 2) {
    throw new Error("Invalid BidCommitted event — expected ≥ 2 topics");
  }
  const auctionId = BigInt(bytesToHex(topics[1]));
  const commitHash = topics.length >= 3 ? bytesToHex(topics[2]) : "0x";
  const [sourceChain] = decodeAbiParameters([
    { name: "sourceChain", type: "uint64" }
  ], bytesToHex(payload.data));
  const chainLabel = sourceChain === 0n ? "local" : `chain:${sourceChain}`;
  runtime2.log(`\uD83D\uDD12 BidCommitted: auction=#${auctionId} commit=${commitHash.slice(0, 18)}… source=${chainLabel}`);
  const auction = getAuction(runtime2, auctionId);
  runtime2.log(`Auction #${auctionId} privacy=${["FULL_PRIVATE", "AUDITABLE"][auction.privacyLevel] ?? auction.privacyLevel}`);
  const bidCount = getBidCount(runtime2, auctionId);
  if (bidCount > 0n) {
    try {
      const latestBid = getBidCommitment(runtime2, auctionId, bidCount - 1n);
      if (latestBid.ipfsCid && latestBid.ipfsCid !== "") {
        const cidStr = latestBid.ipfsCid;
        runtime2.log(`Validating encrypted bid envelope from IPFS: ${cidStr}`);
        const envelope = fetchBidEnvelopeFromIpfs(runtime2, cidStr);
        if (envelope.encryptedPayload) {
          const metadata = decryptMetadata(runtime2, envelope.encryptedPayload);
          runtime2.log(`✅ Validated bid metadata: bidder=${metadata.bidder} token=${metadata.paymentToken}` + (metadata.destinationAddress ? ` dest=${metadata.destinationAddress}` : ""));
        } else if (envelope.encryptedData && envelope.bidder) {
          runtime2.log(`✅ Bid envelope is v2 (frontend wallet-encrypted) — bidder=${envelope.bidder}`);
        } else {
          runtime2.log(`⚠️ Unknown envelope format (version=${envelope.version})`);
        }
      }
    } catch (err) {
      runtime2.log(`⚠️ Failed to validate bid metadata: ${err}`);
    }
  }
  return `committed:${auctionId}:${chainLabel}`;
};
var initWorkflow = (config, _secrets) => {
